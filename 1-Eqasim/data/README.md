...
- This repository keeps **code and data separated**. Do **not** place large raw datasets inside the cloned code repository by default. 
- Most datasets are **national** (download once; the pipeline filters later).
- Some datasets are **department-split** (download only the departments covering your study area). 

## Required datasets (Eqasim reference)

Eqasim lists the following inputs and where they should be stored: 
...
- Addresses database (BAN, department split) → `data/ban_<area>/` 
